...
*** This is a modification of ppdisk.py that comes with the radmc3dPy package
...
def getDustTemperature(grid=None, ppar=None):
    """Calculates the dust temperature distribution in a protoplanetary disk.
       Follows prescription in Williams & Best 2014 which in turn comes from Dartois et al. 2003
    ...
    # hardwired parameter from Dartois et al. 2003
    delta = 2

    # Define cylindrical coordinates and pressure scale height as in getGasDensity
    # note that this is default xy-indexing so we swapaxes later when inserting into arrays
    rr, th = np.meshgrid(grid.x, grid.y)
    zz = rr * np.cos(th)
    rcyl = rr * np.sin(th)

    hp = np.zeros([grid.nx, grid.ny, grid.nz], dtype=np.float64)
    Tmid = np.zeros([grid.nx, grid.ny, grid.nz], dtype=np.float64)
    Tatm = np.zeros([grid.nx, grid.ny, grid.nz], dtype=np.float64)
    hp_r = ppar['hrdisk'] * (rcyl/ppar['hrpivot'])**ppar['plh'] * rcyl
    Tmid_r = ppar['Tmid'] * (rcyl / (1*au))**ppar['plT']
    Tatm_r = ppar['Tatm'] * (rcyl / (1*au))**ppar['plT']
    for iz in range(grid.nz):
        hp[:, :, iz] = hp_r.swapaxes(0, 1)
        Tmid[:, :, iz] = Tmid_r.swapaxes(0, 1)
        Tatm[:, :, iz] = Tatm_r.swapaxes(0, 1)

    T = np.zeros([grid.nx, grid.ny, grid.nz, 1], dtype=np.float64)
    for iz in range(grid.nz):
